...
knitr::opts_chunk$set(echo=F, fig.width=9, warning=F, message=F )
```
...
```{r, results="asis"}
data.frame( ave.caffeine, ave.heart, predicted.heart, difference=residual ) %>%
...
**True Average Dist Between Data and Mean:**
...
jplot( caffeine[1:10], heart.rate.control[1:10], 
       lab1="Caffeine", lab2="Heart Rate", 
       draw.line=F, ylim=c(40,112) )

y.hat <- mean(heart.rate.control)
abline( h=y.hat, col="darkgray", lwd=2 )
segments( x0=caffeine[1:10], y0=y.hat, y1=heart.rate.control[1:10], col="firebrick", lwd=2 )

position <- ifelse( heart.rate.control[1:10] > y.hat, 3, 1 )
text( caffeine[1:10], heart.rate.control[1:10], 
...
```{r}
# sd( heart.rate.control[1:10] )
# mean( abs( heart.rate.control[1:10] - mean(heart.rate.control[1:10]) ) )

abs( heart.rate.control[1:10] - mean(heart.rate.control[1:10]) ) 
```
