...
jplot( caffeine, heart.rate, lab1="Average Caffeine Consumed by Group", 
       lab2="Average Heart Rate by Group", 
       draw.line=F, xaxt="n" )

abline( v=seq( from=0, to=500, by=50 ), col="firebrick", lty=3 )

points( ave.caffeine, ave.heart, pch=19, col="firebrick", cex=2, type="b" )
text( ave.caffeine, ave.heart, round(ave.heart,0), cex=0.9, col="firebrick", pos=3 )

text( ave.caffeine, 40, ave.caffeine, cex=1.5 )
```
...
```{r, results="asis"}
data.frame( ave.caffeine, ave.heart, predicted.heart ) %>%
...
text( ave.caffeine, ave.heart, round(ave.heart,0), pos=3, cex=1.2 )

text( 450, mean(heart.rate), "Mean of Y", pos=3, col="steelblue", cex=1.5 )
```
...
$b_1$ = cov(caffeine,heart.rate) / var(caffeine) = **`r round( cov(caffeine,heart.rate) / var(caffeine), 2 )`**
...
y.bar <- mean(y)
abline( h=y.hat, col="steelblue", lwd=3 )

col.dev <- rep("darkgray",10)
col.dev[7] <- "firebrick"

segments( x0=1:10, y0=y.bar, y1=y, col=col.dev, lwd=3 )
points( x=c(7,7), y=c(y.bar,y[7]), pch=19, col="firebrick", cex=1.5 )
text( x=c(7,7), y=c(y.bar-3,y[7]+3), round(c(y.bar,y[7]),0), pos=c(1,3), col="firebrick", cex=1.5 )


text( 7, y[7]+15, expression(deviation[i] ), col="firebrick", cex=2, pos=3 )
